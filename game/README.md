...
4명 전투원과 9개 구역의 실시간 3D 배경·2D 전투를 결합한 웹 액션 서바이버입니다. 이 문서는 현재 실행·제작·검증 경로를 안내합니다. 게임 소개와 전체 스크린샷은 [루트 README](../README.md)에서 확인할 수 있습니다.
...
- 새로운 슬롯은 이지스의 구조 작전 프롤로그로 시작합니다. 최초 구역 클리어 뒤 추론핵 분석·전투원 합류·외곽 항로 발견을 CG와 대사로 보여 줍니다. 헤이븐 **작전 기록**에서 해금된 10편을 다시 볼 수 있으며 보상은 중복 지급하지 않습니다.
...
- 9개 구역과 보스방은 3D 배경을 공유하며 테마별 구조물이 다릅니다. 지정된 소형 구조물은 파괴 후 이동·사격 경로를 열고, 처치 수나 경험치는 주지 않습니다.
...
외곽 04–06을 모두 클리어하면 07구역이 열리며, 07 → 08 → 09 순서로 진행합니다. 기존 저장에서도 조건을 만족하면 바로 출격할 수 있습니다.

| 구역 | 진행 방식 | 보스와 대응 |
| --- | --- | --- |
| 07 일식 중계항 | 중계기 3곳을 자유 순서로 점령하고 중앙에서 송신. 적이 가까우면 점령이 멈춥니다. | 헬리오스 심판관: 피난 원 진입, 교차 포격 회피, 방향이 바뀌는 태양 광선 |
| 08 방주 운송로 | 생존자 방주를 호위하며 고속·위험 항로와 정비·수리 항로 중 선택. 선체 파괴 시 실패합니다. | 크로노 레비아탄: 철로 탄막의 빈 차선, 지연 기뢰, 고리의 틈 통과 |
| 09 주권의 왕좌 | 인증 키 3개를 한 개씩 회수·운반한 뒤 중앙 접속대를 지킵니다. | 널 소버린: 판결 격자, 이전 이동 위치 재공격, 순차 피난 구역 |

이 구역들은 처치 수 대신 작전 목표로 보스전을 엽니다. 보스는 체력 70%·38%에서 변신하며, 공격 전 경고와 회피 후 반격 기회를 제공합니다. 신규 장면 일러스트 3장, 보스 동작 시트 3장, 3D 지형·작전 오브젝트와 4편의 스토리를 연결했습니다. 게임 이미지는 비공개로 보관하고 배포본에 포함합니다.
