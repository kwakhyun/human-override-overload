...
프로젝트 전용 내장 OpenAI 이미지 도구로 타이틀 1장, 애니메이션풍 상반신 8장, 승인된 스토리 시안 9장을 제작했다. 상반신은 사용자가 명시적으로 승인한 Python 후처리로 실제 알파를 만들고 WebP로 최적화했다. 반실사/체크무늬 중간 시안은 사용하지 않는다. 정확한 프롬프트·보관 원본·해시·산출물은 [제작 명세](docs/art/anime-2026-09-09.json), 연출과 검증은 [작업 기록](docs/art/anime-and-story-2026-09-09.md)에 기록한다. 특정 이미지 모델 버전은 도구 응답으로 확인되지 않아 표기하지 않는다.
