...
test("portrait touch runtime uses ENVELOP presentation and deterministic nearest-target auto aim", async () => {
  const [platform, createGame, scene] = await Promise.all([
  ...
  assert.match(createGame, /new OverloadScene\([\s\S]*mobileRuntime\.autoAim\)/);
...
test("portrait combat owns the full safe viewport and exposes a touch-anywhere floating joystick", async () => {
  ...
  assert.match(portrait, /\.region-select-screen:not\(\.is-cluster-map\) \.region-card-grid \{[\s\S]*grid-template-columns: repeat\(3, 82vw\);[\s\S]*scroll-snap-type: x mandatory/);
});
