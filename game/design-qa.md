...
- 07 중계기 점령·송신, 08 고속/정비 분기 방주 호위, 09 단일 키 회수·운반·중앙 접속을 결정론 엔진에 구현했다. 04–06 완료 저장에서 07→08→09가 순서대로 열린다. 처치 전멸·헌터 전용 초반 공세는 01–06에만 적용한다.
- 신규 보스 3명은 총 9패턴과 70%/38% 변신을 사용한다. 경고 도형과 피해 판정은 같은 데이터이며, 회피 후 2배 피해 반격 시간이 생긴다. 기존 번호 폭탄은 적용하지 않는다.
- 생성 장면 CG 3장·8포즈 보스 원본 3장을 제작하고, 런타임 15파일(약 5.16MiB), 3D 테마·작전 오브젝트와 연결했다. 제작 레시피와 원문 프롬프트는 docs/art/terminal-orbit-*.json에 기록했다. 이미지는 비공개이고 배포 아카이브에만 포함한다.
- npm test: 454/454 통과. 목표·방해·분기·선체 파괴·재시도·타이머 정지·9패턴 수명·해금·재접속·중복 보상 방지를 포함한다. 기존 6구역과 방어전 회귀 검사는 유지했다.
- 실제 60Hz 엔진의 이동·조준·스킬 입력 봇으로 07, 08 두 분기, 09를 클리어했다. HP·좌표·목표 완료를 강제로 바꾸지 않았다. 최대 영구 강화와 모든 수동 스킬을 가진 저장으로 실행한 내부 시뮬레이션이며 외부 플레이어 밸런스 근거가 아니다.
- Edge 실제 React→Phaser 출격에서 07→08→09 클리어, 보스 9패턴·3단계 변신, onFinish 저장, 재접속 다음 구역 해금을 확인했다. 390×844에서는 07 완주, 세 구역 목표 표시, 844×390에서는 08 호위 안내를 검수했다. 콘솔 예외·에셋 HTTP 오류 0건. 검수용 접근자는 Playwright 응답에만 주입하며 제품 소스에 존재하지 않는다.
- 공개 환경에서 신규 구역의 초기 6초 및 보스전에도 읽기 쉬운 한 줄 스킬 바를 유지하도록 했다. 가속 검수의 자동 간소화 이후 화면뿐 아니라 첫 진입 시점의 모든 버튼이 뷰포트 안에 들어오는지도 검사한다.
- 점검 중 고정 전멸형 초반 규칙이 새 점령 작전에 적용되던 문제, 작전 완료의 전멸 문구, 모바일 HUD 겹침을 수정했다. 목표 방향·거리·선체 상태와 미니맵 목표 표식, 보스 회피 조언·반격 시간을 제공한다.
- npm run build 및 Sites Worker 7개 검사 통과. 에셋 소유권은 250개, 누락·미사용 0개다. 공개 Git 이미지 검사 통과. 게임 가이드·AI 보고서·기획 기준선·포트폴리오 PDF 4개를 갱신하고 새 캠페인 페이지를 렌더링해 확인했다.
- 검수 원자료: qa/terminal-orbit/simulation.json, qa/terminal-orbit/browser/evidence.json 및 같은 폴더의 화면·배치 JSON. 실물 모바일 성능, 일반 유저의 난이도·성공률은 측정하지 않았다. 아래 오래된 반복 기록의 07–09 미구현 표기는 이 현행 규칙으로 대체한다.
...
- Asset delivery: production references use a Worker-owned `/cdn/assets/overload/*` namespace so the Sites static fast path cannot bypass the R2 layer. The Worker maps misses back to packaged `/assets/overload/*`, warms R2 asynchronously, and keeps byte-range requests on the native Sites origin for correct video/audio seeking.
...
## 전투 스킬 사용 가능 상태 개선 · 2026-09-07

- 모바일에서 사라졌던 스킬 이름과 상태를 복원하고, 중복된 작은 쿨타임 배지 대신 큰 남은 시간과 하단 충전 막대를 적용했다. PC는 20px, 모바일은 18px 숫자를 사용한다.
- 사용 가능은 체크·밝은 테두리, 대기는 시계·숫자, 미해금은 자물쇠·점선으로 구분한다. 대상 없음과 행동 불가를 쿨타임과 분리했다. 양수 타이머를 0.0초로 반올림하거나 실제로 발동할 수 없는 순간에 준비 완료로 표시하지 않는다.
- 준비 완료 강조는 한 번만 재생하고 OS·게임의 모션 최소화 설정을 따른다. 일시정지 중 자체 타이머를 돌리지 않고, 태그하면 전투원의 실제 스킬 은행을 그대로 표시한다.
- 모바일은 다섯 스킬과 체력 행의 태그 버튼을 유지한다. 기존 가로 화면의 상하 앵커 충돌과 태그로 생기던 추가 행을 수정하고, 이동 안내가 스킬 위에 겹치지 않게 했다.
- 검증: HUD·쿨타임 표시·편성 집중 테스트 15개 및 TypeScript 통과. 실제 App에서 1440×900, 390×844, 844×390의 시전, 대시 재충전, 일시정지, 두 명 교대, 버튼 경계와 모션 최소화를 확인했다. 증거는 `qa/skill-readiness/`, 재현 스크립트는 `scripts/verify-skill-readiness.mjs`이다. 전투 규칙·자산·배포는 변경하지 않았다.
