...
밝은 메뉴음과 아케이드 발사음을 수정했습니다. **현재 구성은 실제 녹음 기반 25개 + Kenney 13개, 총 38개/1,644,252바이트**입니다. AR-15·AK-47·모신나강·SKS 총격, 폭죽 녹음을 가공한 폭발, 낮고 건조한 기계 조작음을 사용합니다. 첫 클릭의 합성 대체음도 비음정 저역 클릭으로 변경했습니다. [새 출처·편집·검증 기록](realistic-sfx-sources.md).

아래는 최초 Kenney 반입 기록이며, 현재 파일별 출처는 `sfx-sources.json`이 기준입니다.
...
전체 자동 테스트 419개, Sites Worker 검사 7개, TypeScript 및 production 빌드를 통과했습니다. 에셋 소유권 검사는 247개, 누락 0, 미사용 0입니다. 실제 Edge 브라우저에서 38개 파일을 모두 해독하고 52개 효과음 항목을 재생했으며, AEGIS 출격의 자동 사격·대시 버튼·EMP·보호막·일시정지/재개와 데스크톱/세로 화면을 확인했습니다. 런타임 오류와 음원 HTTP 실패는 0입니다. 전투 흐름 검사는 기존 디버그 안내 생략 경로를 사용했으며 전체 캠페인이나 실물 모바일의 청감 검증은 아닙니다.
