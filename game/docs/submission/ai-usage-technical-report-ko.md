...
**플레이:** [공개 웹 빌드](https://train-me-wrong-nan2026.khyun97.chatgpt.site)
**소스:** [GitHub 공개 저장소](https://github.com/kwakhyun/human-override-overload)
...
| ChatGPT / OpenAI ImageGen | 캐릭터, NPC, 적, 동료, 보스, 맵, UI 버튼, 픽셀 VFX, 모션 아틀라스 | 프로젝트 전용 이미지 에셋 |
...
| Google Cloud TTS | Q/E/F/R 탑재 AI 에이전트의 짧은 한국어 안내 합성 | 정적 MP3 4개 |
...
- GPU 메모리와 프레임 P95 측정 자동화
- 저사양 품질 정책과 경량 에셋 생성
- CREDITS, README, 설계 결정, 인수인계 문서 지속 갱신
- Git 논리 커밋, Sites 공개 배포, 제출 PDF 제작
...
> “NAN 2026 해커톤에 사전 과제로 제출할 고퀄리티 웹 게임 기획안을 몇가지 제시해줘.”

Codex는 제출 조건을 구조화하고 여러 게임 후보, 핵심 루프, AI 활용 포인트와 제작 범위를 비교했습니다. 초기 제목 `TRAIN ME WRONG`을 선택한 뒤 프로토타입을 만들었지만, 이후 실제 게임 내용이 달라지자 사용자는 제목 적합성을 다시 검토하고 최종적으로 `HUMAN OVERRIDE: OVERLOAD`로 변경했습니다. 이름 역시 최초 제안을 고정하지 않고 제품 변화에 맞춰 다시 판단한 사례입니다.
...
월드를 13,200×1,080으로 확장하고 실제 12,000 유닛을 전진하도록 바꿨습니다. 진행률은 배경 스크롤 변수가 아니라 플레이어의 실제 월드 X 좌표로 계산했습니다. 세 구간 배경, 5개 스폰 게이트, 아군 흔적, 잠긴 보스 격벽, 전멸 후 자동 보스방 전환이 하나의 캠페인 흐름으로 연결됐습니다.

### 사례 D · 탑다운 캐릭터 시점 교정

> “기존 게임 방식과 시점을 유지하려면 캐릭터 스프라이트를 머리 위 하늘에서 보이는 시점으로 제작해야 된다.”

ImageGen 프롬프트에 `strict 90-degree nadir orthographic`, `no visible face`, `centered pivot` 같은 제작 계약을 넣었습니다. 생성 결과를 크로마 제거하고 셀별 공통 앵커로 정규화했습니다. 이후 사용자가 360도 회전 시 부자연스러움을 지적하자 전체 스프라이트 회전을 폐기하고, 머리는 항상 위·발은 아래를 향한 상태에서 상/수평/하 조준 행과 좌우 반전만 사용하는 8×3 아틀라스로 교체했습니다.

### 사례 E · 저품질 레이저 실패를 에셋 파이프라인으로 해결

> “오메가 레이저 공격이 포신 이미지가 길게 늘어지는 모션으로 바뀌어서 굉장히 형편없다. 레이저 공격 전용 그래픽 애니메이션을 만들어서 적용해라.”

긴 포신 한 장을 늘이는 방식이 문제라는 점을 확인하고, 발사기·반복 가능한 빔 코어·종단 충돌을 서로 다른 정사각 모듈로 분리했습니다. 이후 사용자가 과도한 고해상도 이미지가 스킬에 어울리지 않는다고 판단해, 수동·자동 스킬은 64px 16-bit 도트 아틀라스로 다시 만들었습니다. 엔진의 긴 선·캡슐 판정은 Graphics가 그리고 작은 도트 모듈은 반복 배치해 이미지 왜곡을 없앴습니다.
...
- 첫 지역은 정확히 300기, 후속 지역은 1,000기
- 적은 빈 공간이 아니라 5개 전송 게이트에서만 생성
- 기본 공격은 상시 자동, Q/E/F/R은 독립 수동 스킬
- 플레이어는 화면 중앙, 머리 위·발 아래, 좌우 반전만 사용
- 일반전과 보스방은 독립 씬 자산·좌표계로 분리
- 패링은 1.5초, 시한폭탄은 2/4/8개와 10/14/22초
- 저사양 품질 저하는 판정·입력·경고 geometry를 바꾸지 않음
- 현재 캐릭터 대사는 텍스트이며 브라우저 TTS·음성 복제 금지
- Q/E/F/R 성공 안내만 Google Cloud 정적 MP3로 허용
...
- React 타이틀, 저장 슬롯, 기지, NPC, 연구실, 정비소, 지역 선택, 영상, HUD, 가이드, 결과 화면
- 결정론 적 스폰, 병과, 투사체, 회복 키트, 레벨업, 수동·자동 스킬, 보스 패턴, 패링, 폭탄
...
- 은발 AEGIS 캐릭터 포트레이트와 탑다운 조준·이동 모션
- RHEA, HANA, ILYA, LARK NPC 일러스트
- 자폭 드론, 소총수, 저격 플랫폼과 동료 드론·센트리
- THE WRONG ENGINE, MIRROR TYRANT, DROWNED ORACLE의 단계별 형상과 모션
- 세 지역 전장, 독립 보스방, HAVEN-09, 비행선 지역 지도
- 미래형 명령 버튼 3상태 아틀라스
...
각 영상은 1264×720, 24fps, 약 6.04초 MP4입니다. 런타임에는 원본을 재인코딩하지 않고 byte-identical하게 이름만 정리해 포함했습니다. 지역 선택 화면에서 출격을 최종 확인한 뒤 선택한 영상 하나만 `preload="metadata"`로 마운트합니다. 기지 BGM은 영상 동안 멈추고, `ended` 이벤트 직후 Phaser 전투와 해당 지역 BGM을 시작합니다. 미디어 오류나 자동재생 제한이 발생하면 전투를 막지 않고 안전하게 진행합니다.

Grok 영상은 실제 플레이 영상으로 제출하는 것이 아닙니다. 30~60초 제출 영상은 게임 화면을 직접 캡처하고, Grok 영상은 게임 안의 짧은 출격 시네마틱으로만 사용합니다.
...
전체 캐릭터 대사를 브라우저 TTS로 읽게 했을 때 음질과 장치별 편차가 좋지 않아 해당 기능을 완전히 제거했습니다. 대신 전투 중 필요한 Q/E/F/R **탑재 AI 에이전트 성공 안내**만 Google Cloud Text-to-Speech로 오프라인 생성했습니다.
...
처음에는 음성이 길고 발동보다 늦게 들리는 문제가 있었습니다. 사용자는 “TTS가 너무 길다. 뒤에 설명 내용은 제거하고 짧게 수정”, “반응과 음성 속도가 너무 느리다”고 피드백했습니다. Codex는 설명 문장을 제거하고 발화 속도를 1.3으로 올린 뒤, 전투 진입 때 `audio.load()`를 명시적으로 요청해 스킬 성공 이벤트 즉시 재생되도록 했습니다. 최종 길이는 약 0.984~1.680초입니다.

음성 우선순위는 R > F > E > Q로 두어 더 중요한 스킬 안내가 약한 안내에 끊기지 않습니다. 전체 사운드 토글을 따르며 파일 로드가 실패하면 조용히 넘어갑니다.

## 11. 결정론 전투 AI와 생성형 AI의 구분

게임의 세계관은 초지능 SOVEREIGN과 그 기계 군단을 다루지만, 현재 전투 중 적 행동은 서버 LLM이나 생성 API를 호출하지 않습니다. 대규모 300/1,000기 전투에서 재현성·성능·공정한 경고를 보장하기 위해 결정론 시뮬레이션을 사용합니다.
...
이 피드백은 수치와 계약으로 바뀌었습니다. 예를 들어 플레이어 74px, 적 92/108/138px, 흔적 54/58/62px, 첫 지역 적 300기, 화면 안 저격 경고 cap 3→5→7, 패링 1.5초, 폭탄 10/14/22초 같은 값이 실제 플레이 결과에 따라 조정됐습니다.
...
- Playwright/Edge로 데스크톱·모바일 가로 스크린샷과 콘솔 오류 확인
- PNG 헤더·알파·셀 nonempty·경계 overflow 검사
- Sites Worker의 정적 자산·SPA fallback 검증

프로젝트 주요 체크포인트에서는 101/101, 158/158, 181/181, 198/198 등 당시 전체 회귀 테스트가 통과했습니다. 마감 직전에는 사용자의 시간 우선 지시에 따라 전체 suite를 매번 돌리지 않고 변경 범위의 focused test와 typecheck만 수행했습니다.
...
첫 장면에서 적이 너무 많아 성장과 전진의 변화가 약했습니다. 진행도 10/34/60/82% 증원, 레벨·처치·시간 보조 압력, 5게이트 경고 스폰으로 바꿨고, 첫 지역 총량도 1,000기에서 300기로 완화했습니다.
...
- Google Cloud TTS: Q/E/F/R 정적 시스템 음성 4개

영상과 BGM 런타임 파일은 사용자 제공 원본을 재인코딩하지 않은 byte-identical 사본입니다. SHA-256과 길이는 CREDITS에 기록했습니다.
...
- AI 생성 원본과 파생 스크립트는 제작 기록으로 로컬에 보존하되, 공개 저장소에는 실행에 필요한 소스·런타임 에셋·문서만 선별합니다.
...
> “보스가 맵 전역에 광범위로 폭발하는 시한폭탄을 2개, 4개, 8개 순서대로 설치하고 플레이어는 시한폭탄 위에 적힌 숫자 순서대로 클릭해서 폭탄을 제거.”
...
3. **실패한 AI 결과를 버리는 판단력**
   잠입 게임, 4구역 지휘, 고해상도 스킬 VFX, 전체 브라우저 TTS 등 완성도가 낮거나 방향이 다른 결과를 과감히 폐기했습니다.
...
5. **검증 가능한 결과를 만드는 능력**
   자동 테스트, 브라우저 캡처, frame P95, GPU texture memory, 해시, CREDITS, Git 커밋과 공개 배포를 통해 산출물을 추적할 수 있습니다.
...
- 공개 플레이: https://train-me-wrong-nan2026.khyun97.chatgpt.site
- 공개 소스: https://github.com/kwakhyun/human-override-overload
