...
공개 GitHub 소스와 게임의 에셋 제공 범위는 다릅니다. 사용자가 공개를 승인한 대표 아이콘 3종 외의 게임 이미지·Cubism 모델·제작 입력은 Git 추적에서 제외하며, 권한 있는 비공개 사본을 로컬 경로에 복원한 뒤 빌드합니다. Sites에는 이미지가 포함된 실행 산출물을 배포합니다. 원격 소스만으로 재빌드하려면 같은 비공개 에셋 입력이 필요합니다. [이미지 보관 정책](public-repository-assets.md)을 참고하세요.
