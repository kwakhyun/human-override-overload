...
초기 13,200×1,080 전진형 월드를 다시 26,400×1,080으로 확장하고 실제 전진 거리를 25,000 유닛으로 늘렸습니다. 진행률은 배경 스크롤 변수가 아니라 플레이어의 실제 월드 X 좌표로 계산했습니다. 지역별 대표 배경과 확장 섹터, 5개 스폰 게이트, 아군 흔적, 처치 기반 웨이브와 전멸 후 자동 보스방 전환을 하나의 캠페인 흐름으로 연결했습니다. 이후 맵 끝 입구·잠금벽은 제거하고 모든 적 처치 자체가 다음 단계의 권위가 되도록 단순화했습니다.
...
프로젝트 주요 체크포인트에서는 101/101, 158/158, 181/181, 198/198 등 당시 전체 회귀 테스트가 통과했습니다. 2026년 8월 15일 최신 전체 검증에서는 **269/269 Node 테스트**, TypeScript 검사, production build, Sites 4/4가 통과했습니다. 캠페인 01—06, 캐릭터·무기 선택, 태그, 모바일 세로 조이스틱·오버레이, 디펜스 세로 전장과 저장·보상 흐름을 포함해 확인했습니다. 이후 사용자의 시간 우선 지시에 따라 전체 suite를 매번 돌리지 않고 변경 범위의 focused test만 수행합니다.
