...
**소스:** [GitHub 저장소](https://github.com/kwakhyun/human-override-overload) · 소유자 관리 Private
...
초기 13,200×1,080 전진형 월드를 다시 26,400×1,080으로 확장하고 실제 전진 거리를 25,000 유닛으로 늘렸습니다. 진행률은 배경 스크롤 변수가 아니라 플레이어의 실제 월드 X 좌표로 계산했습니다. 지역별 대표 배경과 확장 섹터, 5개 스폰 게이트, 아군 흔적, authored 전투 구간과 전멸 후 자동 보스방 전환을 하나의 캠페인 흐름으로 연결했습니다. 현재 웨이브를 전멸해야 다음 구간까지의 진행 한계가 풀리고, 해당 구간에 도달해야 다음 공세가 열립니다. 전진만으로 적을 건너뛸 수 없고 최종 공세는 종점 전에 배치되며, 맵 끝의 수동 입장문은 사용하지 않습니다.
...
- 후속 관문 공세는 `현재 웨이브 전멸 + authored 전투 구간 도달`을 모두 만족해야 시작
- 적은 빈 공간이 아니라 5개 전송 게이트에서만 생성
- 기본 공격은 상시 자동, Q/E/F/R은 캐릭터·무기별 독립 수동 스킬
...
- AEGIS·MIKA와 소총·검별 기본 공격·Q/E/F/R·태그 시스템
...
- 은발 AEGIS와 핑크 양갈래 MIKA의 포트레이트, 8방향 탑다운 조준·이동·공격 모션
...
전체 캐릭터 대사를 브라우저 TTS로 읽게 했을 때 음질과 장치별 편차가 좋지 않아 해당 기능을 완전히 제거했습니다. 대신 AEGIS 펄스 소총 세트의 Q/E/F/R **탑재 AI 에이전트 성공 안내**만 Google Cloud Text-to-Speech로 오프라인 생성했습니다. 빔 소드와 MIKA는 전용 음원이 없어 잘못된 이름을 읽는 대신 무음으로 동작합니다.
...
프로젝트 주요 체크포인트에서는 101/101, 158/158, 181/181, 198/198 등 당시 전체 회귀 테스트가 통과했습니다. 마지막 전체 검증 체크포인트인 2026년 8월 15일에는 **269/269 Node 테스트**, TypeScript 검사, production build, Sites 4/4가 통과했습니다. 캠페인 01—06, 캐릭터·무기 선택, 태그, 모바일 세로 조이스틱·오버레이, 디펜스 세로 전장과 저장·보상 흐름을 포함해 확인했습니다. 이후에는 변경 범위의 focused test만 수행했으므로, 269/269를 현재 전체 suite의 통과 선언으로 사용하지 않습니다.
...
첫 장면에서 적이 너무 많아 성장과 전진의 변화가 약했습니다. 초기 개선안에서는 진행도 10/34/60/82%만으로 증원했지만, 살아 있는 적을 남긴 채 다음 공세가 겹칠 수 있었습니다. 이후 처치만을 조건으로 삼자 맵 끝에서 장시간 정지 전투가 생겼습니다. 현재 런타임은 근접형 8기로 시작하고 `현재 웨이브 전멸 + 다음 authored 전투 구간 도달`을 모두 만족할 때 `14→22→34→52→79→120→180→220`기 관문 공세를 순차 배치합니다. 첫 지역 총량은 1,000기에서 300기로 완화했고, 최종 공세는 종점 전에 각 지역 예산의 잔여 수량을 정확히 생성합니다.

### 실패 6 · 눈에 보이지 않는 프록시 Live2D

Cubism 모델·모션·물리 파일과 WebGL 캔버스는 로드됐지만 AEGIS 얼굴 분할선, MIKA의 어색한 전신 구도와 육안으로 확인하기 어려운 변형을 해결하지 못했습니다. CSS 위치 이동·홍조·얼굴선으로 움직임을 가장하지 않고 활성 Live2D 런타임과 의존성을 제거했습니다. 현재 로비는 승인된 AEGIS·MIKA 원본 정적 일러스트와 투명 터치 영역별 대사만 사용하며, Cubism 자료는 향후 전문 리깅을 위한 비활성 제작 이력으로 보존합니다.
...
- 소스 저장소: https://github.com/kwakhyun/human-override-overload · 소유자 관리 Private
