...
| `E` | 방벽 전개 - 회복·보호막·상태 방어장 |
| `F` | 항공 지원 - 세 평행 항로 지원 폭격 |
| `R` | 섬멸 모드 - 360도 회전 필살기 |
...
포인터를 통과하는 세 평행 항로에 순차 항공 지원을 요청합니다. 적이 없는 지점에도 미리 발동할 수 있으며, 경고 항로를 따라 다중 폭격이 지면을 연속으로 초토화합니다. 경고 항로와 실제 공격 판정은 일치합니다.
...
보스 체력 55%·30%·12%에서 전역 사이렌과 함께 폭탄 2·3·4개가 설치됩니다. 각각 12·16·20초 안에 폭탄 위 숫자를 **1부터 순서대로 클릭**해야 합니다. 순서를 틀리거나 시간이 끝나면 남은 폭탄이 즉시 전부 폭발하고, 9초 동안 보스가 받는 피해가 16%로 감소합니다.
