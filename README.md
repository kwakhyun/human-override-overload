...
- [프로젝트 정리 기록](game/docs/maintenance/project-cleanup-2026-09-07.md)
...
현재 GitHub 원격 저장소는 소유자가 직접 **Private**로 운영합니다. 실행 소스·활성 런타임 에셋과 재현에 필요한 제작 계약을 버전 관리하며, 현재 사용하지 않는 공개 에셋은 제거하고 필요한 제작 입력은 `reference/`로 분리했습니다. 빌드는 공개 에셋의 누락과 미사용 파일을 먼저 검사합니다. 로컬 QA 캡처, 생성 중간본, 브라우저 프로파일, 세션 인계 파일과 인증 정보도 배포 대상이 아닙니다. 프로젝트 전용 AI 생성 에셋의 개별 재배포·재판매는 허용되지 않으며, 오픈소스 의존성은 각 원 라이선스를 따릅니다.
